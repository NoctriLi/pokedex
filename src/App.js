...
const fadeOut = [{ opacity: 1 }, { opacity: 0 }, { opacity: 0 }];
const fadeIn = [{ opacity: 0 }, { opacity: 0 }, { opacity: 1 }];
const timing = {
  duration: 2000,
  iterations: 1,
};
...
class Dex extends React.Component {
      ...
      fetch("https://pokeapi.co/api/v2/pokemon/" + poke)
        ...
        .then((data) => {
          ...
          this.setState({
            ...
            type:
              data.types.length === 2
                ? capitalizeFirst(data.types[0].type.name) +
                  "/" +
                  capitalizeFirst(data.types[1].type.name)
                : capitalizeFirst(data.types[0].type.name),
          });
        })
        .catch((err) => {
  ...
  handleClick() {
    if (this.state.show) {
      ...
      setTimeout(() => {
        document.getElementById("screen-container").style.opacity = 0;
      }, 1000);
  ...
  render() {
    ...
    let pHeight = window.innerHeight * 0.9;
    let pWidth = pHeight / (16 / 9);
    let sHeight = (pHeight * 0.9).toString() + "px";
    let bHeight = (pHeight * 0.05).toString() + "px";
    pHeight = pHeight.toString() + "px";
    pWidth = pWidth.toString() + "px";

    return (
      <div
        className=" mx-auto position-static top-100"
        style={{
          height: "100%",
          width: "100%",
          background:
            "linear-gradient(0deg, rgba(19,109,21,1) 0%, rgba(29,145,31,1) 14%",
        }}
      >
        <div
          className=" mx-auto"
          style={{
            ...
            width: "100%",
            background:
              "linear-gradient(0deg, rgba(29,145,31,1) 0%, rgba(31,210,253,1) 11%, rgba(235,243,245,1) 90%)",
          }}
        >
          <div className="position-absolute top-50 start-50 translate-middle mx-auto">
            ...
            >
              ...
              >
                <div id="screen-container" className="fadeIn">
                  ...
                  >
                    <div id="gen-info" className="row">
                      <p id="generation" className="text-center">
